...
@Injectable()
export class TwitterTransactionService {
  private readonly logger = new Logger(TwitterTransactionService.name)
  ...
  private async initClient() {
    const res = await this.clientLimiter.schedule(async () => {
      if (!this.transaction) {
        this.logger.log('initClient: create')
        const document = await handleXMigration()
        const transaction = await ClientTransaction.create(document)
        this.transaction = transaction
        this.logger.log('initClient: ok')
      }
